...
Adapted from [Dalamud.LoadingImage](https://github.com/MapleHinata/Dalamud.LoadingImage) by goat / Maple. The original used a byte-signature hook on the game's territory-change function; this port resolves the destination by polling `GameMain.NextTerritoryTypeId` (with `TransitionTerritoryTypeId` and `IClientState.TerritoryType` as fallbacks) on each `_LocationTitle.PreDraw` until one returns non-zero. No signature to maintain across patches.
