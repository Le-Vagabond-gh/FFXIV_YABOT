...
The **Show kill counter** option shows how many enemies you've killed on the current floor against the floor's kill-to-open-passage range (`Kills  3 / 3-7`), turning green once the passage is open. The Beacon/Cairn of Passage opens after a number of kills that is **rolled randomly per floor within a fixed per-floorset range** (data from [ddcompendium.com](https://www.ddcompendium.com/), folded into `DeepDungeonPassage`), and the game never exposes the rolled value - `PassageProgress` is decorative fill that just snaps to `>=11` when the threshold is met. So an exact "X more kills" is impossible; the range is the best guidance available. Killing every enemy on the floor opens it regardless of the roll.

Kills are tallied in `TrackKills`: each frame, the object table is scanned for enemy `IBattleNpc`s (`BattleNpcSubKind` value `5`, compared by byte since the member name differs across Dalamud/FFXIVClientStructs versions) that are `IsDead`, each counted once by `GameObjectId`. The counter resets on floor change alongside the respawn anchor, and **freezes once the passage opens** (`raw >= 11`) so it settles on the kills-to-open value rather than ticking up on cleanup kills.

A **magicite/demiclone summon wipes the whole floor at once** and opens the passage instantly, which isn't a kills-to-open signal. The chat handler watches for its use line (e.g. `You use a splinter of Vortex magicite.`, matched on `magicite`/`demiclone`) and stamps a short window during which `TrackKills` skips the death burst (the wiped enemies are still marked seen, so they're never recounted). The window is cleared on floor change so it can't bleed into the next floor.

The **Debug: show raw passage counter** option adds a `Raw  N` segment showing the unscaled `dd->PassageProgress` byte (shown everywhere, boss floors included). It tracks "progress units" - a Flight kill advances it by 2, a normal kill by less - and snaps to `>=11` once the per-floor threshold is met. While the option is on, every `(seconds-since-entry, kills, raw)` change is recorded into `_passageHistory`, and on the next floor change the leaving floor's full history is auto-dumped to `/xllog` (with dungeon, floor, range and Flight state in the header) so it can be pasted straight out. Logging this with and without Flight is how the kill->units ramp and the hidden threshold get characterised.

**Pomander of Flight** is spent on one floor but takes effect on the *next* one, where each kill counts double toward the passage - and there's no on-floor status to read it back from. The chat handler watches for `You use a pomander of flight.` and records the floor it was used on; the following floor is then treated as Flighted and the displayed range is halved (`ceil(n/2)`, e.g. `3-7` -> `2-4`, shown as `Kills  1 / 2-4 (Flight)`). A non-sequential floor change (fresh entry, or a Pomander of Return) clears the pending Flight, since it only ever carries one floor forward.
